...
USB_NOCACHE_RAM_SECTION chry_ringbuffer_t g_uartrx_uart_ttl;
USB_NOCACHE_RAM_SECTION chry_ringbuffer_t g_usbrx_uart_ttl;
...
uart_data_msg_t  tUartMsgObj_uart_ttl;
volatile uint8_t config_uart_uart_ttl = 0;
volatile struct cdc_line_coding g_cdc_lincoding_ep2_uart_ttl;
volatile uint8_t config_uart_transfer_uart_ttl = 0;
volatile uint8_t usbrx_idle_flag_uart_ttl = 0;
volatile uint8_t usbtx_idle_flag_uart_ttl = 0;
volatile uint8_t uarttx_idle_flag_uart_ttl = 0;
USB_NOCACHE_RAM_SECTION USB_MEM_ALIGNX uint8_t cdc_tmpbuffer_ep2_uart_ttl[DAP_PACKET_SIZE];
    ...
    if (rx_resource->channel == channel) {
        for (i = 0; i < rx_desc_size; i++) {
            if (link_addr == core_local_mem_to_sys_address(BOARD_RUNNING_CORE, (uint32_t)&rx_descriptors[i])) {
                emit(uart_ttl_rx, &tUartMsgObj_uart_ttl,
                     args(
...
SDK_DECLARE_EXT_ISR_M(UART_IRQ, uart_isr_uart_ttl)
void uart_isr_uart_ttl(void)
{
...
{
    chry_ringbuffer_init(&g_uartrx_uart_ttl, uartrx_ringbuffer, CONFIG_UARTRX_RINGBUF_SIZE);
    chry_ringbuffer_init(&g_usbrx_uart_ttl, usbrx_ringbuffer, CONFIG_USBRX_RINGBUF_SIZE);
...
static void chry_dap_usb2uart_uart_send_bydma(uint8_t *data, uint16_t len)
{
    ...
    // printf("Start dma: %X, %d", buf_addr, len);
...
{
    //printf("ttl bulk out busid=%d; ep=%d; nbytes=%d; (%d/%d/%d/%d)\n", busid, ep, nbytes, usbtx_idle_flag_uart_ttl, uarttx_idle_flag_uart_ttl, usbrx_idle_flag_uart_ttl, config_uart_transfer_uart_ttl);
    (void) busid;
    (void) ep;
    led_usb_out_activity();
    chry_ringbuffer_write(&g_usbrx_uart_ttl, cdc_tmpbuffer_ep2_uart_ttl, nbytes);
    //emit(uart_ttl_rx, &tUartMsgObj_uart_ttl,
    //                     args(
    //                          cdc_tmpbuffer_ep2_uart_ttl,
    //                          nbytes
    //                     ));
    if (chry_ringbuffer_get_free(&g_usbrx_uart_ttl) >= DAP_PACKET_SIZE) {
        usbd_ep_start_read(0, ep, cdc_tmpbuffer_ep2_uart_ttl, DAP_PACKET_SIZE);
    } else {
        usbrx_idle_flag_uart_ttl = 1;// usbrx_idle_flag = 1;
    }
...
{
    //printf("ttl bulk in busid=%d; ep=%d; nbytes=%d; (%d/%d/%d/%d)\n", busid, ep, nbytes, usbtx_idle_flag_uart_ttl, uarttx_idle_flag_uart_ttl, usbrx_idle_flag_uart_ttl, config_uart_transfer_uart_ttl);
    (void) busid;
    (void) ep;
    uint32_t size;
    uint8_t *buffer;
    led_usb_in_activity();
    chry_ringbuffer_linear_read_done(&g_uartrx_uart_ttl, nbytes);
    if ((nbytes % DAP_PACKET_SIZE) == 0 && nbytes) {
        /* send zlp */
        usbd_ep_start_write(0, ep, NULL, 0);
    } else {
        if (chry_ringbuffer_get_used(&g_uartrx_uart_ttl)) {
            buffer = chry_ringbuffer_linear_read_setup(&g_uartrx_uart_ttl, &size);
            usbd_ep_start_write(0, ep, buffer, size);
        } else {
            usbtx_idle_flag_uart_ttl = 1; // usbtx_idle_flag = 1;
        }
    }    
}

struct usbd_endpoint cdc_out_ep2_uart_ttl = {
        .ep_addr = CDC_OUT_EP2,
        .ep_cb = usbd_cdc_acm_bulk_out
};

struct usbd_endpoint cdc_in_ep2_uart_ttl = {
        .ep_addr = CDC_IN_EP2,
...
static void chry_dap_usb2uart_uart_send_complete(uint32_t size)
{
    ...
    chry_ringbuffer_linear_read_done(&g_usbrx_uart_ttl, size);

    if (chry_ringbuffer_get_used(&g_usbrx_uart_ttl)) {
        buffer = chry_ringbuffer_linear_read_setup(&g_usbrx_uart_ttl, &size);
        chry_dap_usb2uart_uart_send_bydma(buffer, size);
    } else {
        uarttx_idle_flag_uart_ttl = 1; // uarttx_idle_flag = 1;
    }
}
...
void chry_dap_usb2uart_handle_uart_ttl(void)
{
    ...
    if (config_uart_uart_ttl) {
        /* disable irq here */
        config_uart_uart_ttl = 0;
        /* config uart here */
        chry_dap_usb2uart_uart_config_callback((struct cdc_line_coding *) &g_cdc_lincoding_ep2_uart_ttl);
        usbtx_idle_flag_uart_ttl = 1;
        uarttx_idle_flag_uart_ttl = 1;
        config_uart_transfer_uart_ttl = 1;
        printf("ttl configed; start transfer\n");
        //chry_ringbuffer_reset_read(&g_uartrx);
        /* enable irq here */
    }

    if (config_uart_transfer_uart_ttl == 0) {
        return;
    }
    ...
    */
    ...
    /* uartrx to usb tx */
    if (usbtx_idle_flag_uart_ttl) {
        uint32_t used = chry_ringbuffer_get_used(&g_uartrx_uart_ttl);
        if (used) {
            usbtx_idle_flag_uart_ttl = 0;
            /* start first transfer */
            buffer = chry_ringbuffer_linear_read_setup(&g_uartrx_uart_ttl, &size);
            usbd_ep_start_write(0, CDC_IN_EP2, buffer, size);
            //printf("ttl->usb(%d/%d)\n", size, used);
        }
        // printf("ttl->usb(%d/%d)\n", size, used);
    }
    ...
    /* usbrx to uart tx */
    if (uarttx_idle_flag_uart_ttl) { // 发送有没有结束?
        size_t used = chry_ringbuffer_get_used(&g_usbrx_uart_ttl);
        if (used) {            
    ...
    if (usbrx_idle_flag_uart_ttl) {
        size_t free = chry_ringbuffer_get_free(&g_usbrx_uart_ttl);
        if (free >= DAP_PACKET_SIZE) {
