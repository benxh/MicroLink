...
uint8_t validate_bin_nvic_base(const uint8_t *buf)
{
    ...
    if (1 == test_range(nvic_val, 0x24000000, 0x24080000)) {
        in_range = 1;
    }else  if (1 == test_range(nvic_val, 0x20000000, 0x20080000)) {
        in_range = 1;
    } 
    ...
    for (; i <= 12; i += 4) {
        ...
        if (1 == test_range(nvic_val, 0x08000000, 0x08200000)) {
            in_range = 1;
        }
        if (in_range == 0) {
            return 0;
...
uint8_t validate_rblfile(const uint8_t *buf)
{
    ...
    return (strcmp(s_tPartHeat.chType, "RBL") == 0);
}
