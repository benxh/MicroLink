...
#define HARD_VERSION "V1.2.0"
#define APP_VERSION  "V1.0.2"

#define UPDATA_VERSION \
"V1.0.2 \r\n\
1.u盘拖拽ymodem起始发送ry命令，结束发送reboot命令\r\n\
V1.0.1 \r\n\
1.修复使用串口导致daplink失效的问题\r\n\
V1.0.0 \r\n\
1.支持SWD/JTAG接口，下载速度超越JLINK V12（时钟10Mhz）\r\n\
2.支持使用OpenOCD调试的IDE调试ARM/RISC-V等芯片\r\n\
3.支持USB转串口，最大10M波特率无丢包\r\n\
4.支持大量Cortex-M系列芯片U盘拖拽下载，内置大量下载算法，自动识别目标芯片\r\n\
5.内置ymodem协议栈，U盘拖拽文件（hex、bin）自动触发ymodem通过串口升级目标芯片固件（需配合带ymodem协议的bootloader）\r\n\
6.支持系统固件升级，为后续添加更多功能\r\n\
7.采用winusb对window10免驱，即插即用\r\n\
8.支持3V3/5V大电流输出电源\r\n\
9.内置防倒灌和过流保护，外部电流无法反向流入USB口，防止损坏USB\r\n\
"
...
void vfs_user_build_filesystem()
{
    ...
    vfs_init("MicroLink    ", VFS_DISK_SIZE);
    // MBED.HTM
    file_size = get_file_size(read_file_mbed_htm);
    vfs_create_file("MBED    HTM", read_file_mbed_htm, 0, file_size);
    // DETAILS.TXT
...
static uint32_t update_details_txt_file(uint8_t *buf, uint32_t size, uint32_t start)
{
    ...
    pos += util_write_string_in_region(buf, size, start, pos,
        ...
        // Version
        "Soft Version: " APP_VERSION "\r\n"
        // Build Date:
        "Hard Version: " HARD_VERSION "\r\n");
    ...
    do {
        ...
        //            insert_string = (uint8_t *)info_get_version();
                ...
                case 'R':   // URL replacement
                    insert_string = "https://microboot.readthedocs.io/zh-cn/latest/tools/microlink/microlink/";
                    break;
