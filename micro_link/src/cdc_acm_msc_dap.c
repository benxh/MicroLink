...
usb_osal_sem_t semaphore_acm_tx_done, semaphore_dap_tx_done;
...
void dap_out_callback(uint8_t busid,uint8_t ep, uint32_t nbytes)
{
    ...
    if (USB_Request[USB_RequestIndexI][0] == ID_DAP_TransferAbort) {
        ...
        USB_RequestCountI++;
        if (semaphore_dap_tx_done != NULL) {
            if (usb_osal_sem_give(semaphore_dap_tx_done) != 0) {
    ...
    if ((uint16_t)(USB_RequestCountI - USB_RequestCountO) != DAP_PACKET_COUNT) {
        usbd_ep_start_read(busid,ep, USB_Request[USB_RequestIndexI], DAP_PACKET_SIZE);
    } else {
        USB_RequestIdle = 1U;
    }
}
...
void dap_in_callback(uint8_t busid,uint8_t ep, uint32_t nbytes)
{
    ...
    if (USB_ResponseCountI != USB_ResponseCountO) {
        ...
        usbd_ep_start_write(busid,ep, USB_Response[USB_ResponseIndexO], USB_RespSize[USB_ResponseIndexO]);
        USB_ResponseIndexO++;
...
void usbd_cdc_acm_bulk_out(uint8_t busid,uint8_t ep, uint32_t nbytes)
{
    ...
    if (chry_ringbuffer_get_free(&g_usbrx) >= DAP_PACKET_SIZE) {
        usbd_ep_start_read(busid,ep, usb_tmpbuffer, DAP_PACKET_SIZE);
    } else {
        usbrx_idle_flag = 1;
    }
...
__WEAK void usbd_msc_read_sect(uint32_t block, uint8_t *buf, uint32_t num_of_blocks)
{

}
__WEAK void usbd_msc_write_sect(uint32_t block, uint8_t *buf, uint32_t num_of_blocks)
{
...
void usbd_msc_get_cap(uint8_t busid, uint8_t lun, uint32_t *block_num, uint32_t *block_size)
{
    (void)busid;
    (void)lun;
    *block_num = USBD_MSC_BlockCount; //Pretend having so many buffer,not has actually.
...
int usbd_msc_sector_read(uint8_t busid, uint8_t lun, uint32_t sector, uint8_t *buffer, uint32_t length)
{
    (void)busid;
    (void)lun;
    usbd_msc_read_sect(sector,buffer,(length + (USBD_MSC_BlockSize - 1)) / USBD_MSC_BlockSize);
    return 0;
}
...
int usbd_msc_sector_write(uint8_t busid, uint8_t lun, uint32_t sector, uint8_t *buffer, uint32_t length)
{
    (void)busid;
    (void)lun;
    usbd_msc_write_sect(sector, buffer, (length + (USBD_MSC_BlockSize - 1)) / USBD_MSC_BlockSize);
    return 0;
}
#endif
void chry_dap_init(uint8_t busid, uint32_t reg_base)
{
    ...
    semaphore_dap_tx_done = usb_osal_sem_create(0);
...
void chry_dap_handle(uint8_t busid)
{
    ...
    while(1){
      ...
      usb_osal_sem_take(semaphore_dap_tx_done, 10);
    }
}
...
void chry_dap_usb2uart_handle(uint8_t busid)
{
    ...
    while(1){
      if (config_uart) {
          ...
          CONFIG_USB_PRINTF("\r\n reset baudrate = %d\r\n",g_cdc_lincoding.dwDTERate);
          /* enable irq here */
      }

      if (config_uart_transfer ) {
        /* uartrx to usb tx */
        if (usbtx_idle_flag) {
            if (chry_ringbuffer_get_used(&g_uartrx)) {
                usbtx_idle_flag = 0;
      ...
      usb_osal_sem_take(semaphore_acm_tx_done, 10);
    }
}
