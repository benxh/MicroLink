...
void uart_isr(void)
{
        ...
        if (uart_received_data_count > 0) {
            for (i = 0; i < rx_desc_size; i++) {
                if (link_addr == core_local_mem_to_sys_address(BOARD_RUNNING_CORE, (uint32_t)&rx_descriptors[i])) {
                    ...
                    ymomdem_uart_in_queue(uart_rx_buf[i], uart_received_data_count);
                    board_uart_rx_dma_restart();
                    break;
                }
            }
    ...
    if (uart_is_txline_idle(UART_BASE)) {
        ...
        gpio_write_pin(BOARD_RE_EN_GPIO_CTRL, BOARD_RE_EN_GPIO_INDEX, BOARD_RE_EN_GPIO_PIN, 0);
    }
}
...
void chry_dap_usb2uart_uart_config_callback(struct cdc_line_coding *line_coding)
{
    ...
    config.rxidle_config.threshold = 10U; /* 10bit */
...
void chry_dap_usb2uart_uart_send_bydma(uint8_t *data, uint16_t len)
{
    ...
    gpio_write_pin(BOARD_RE_EN_GPIO_CTRL, BOARD_RE_EN_GPIO_INDEX, BOARD_RE_EN_GPIO_PIN, 1);
    clock_cpu_delay_us(20);
